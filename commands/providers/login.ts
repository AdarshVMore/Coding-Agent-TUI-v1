...
export const loginCommand = new Command("login")
    ...
    .action(async (provider, apiKey) => {
        const data:any = await axios.post("http://localhost:3000/api/login", {provider: provider, apiKey:apiKey});
      apiKey = JSON.stringify(data.data)
      console.log("set provider to ", provider, " here is the api key ", data.data)
